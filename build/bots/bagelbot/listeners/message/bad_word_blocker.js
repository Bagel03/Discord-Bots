...
updateCache();
export const onCalled = async (message) => {
    ...
    if (message.member?.hasPermission("ADMINISTRATOR"))
        return;
    message.delete();
    const msg = await embedGenerator.sendNormalEmbed(message, "Message contained a blocked word", `A message sent by ${message.member?.displayName} contained a word that was blocked`);
