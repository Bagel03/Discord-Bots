...
export const onCalled = async (client, message) => {
    ...
    if (message.channel.type == "dm") {
        embedGenerator.sendNormalEmbed(message, "Please start game in a server", "To play games that need more than one person, you will need to start them in a server");
        return;
    }
    ...
    let otherPlayer;
    try {
        let results = await msg.awaitReactions((reaction, user) => {
