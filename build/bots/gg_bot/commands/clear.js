...
export const onCalled = async (client, message) => {
    ...
    if (num !== NaN) {
        ...
        const msg = await embedGenerator.sendNormalEmbed(message, `Deleted ${num} message`, `Deletion took ${Math.ceil(Date.now() - start)} milliseconds`, []);
        msg.delete({ timeout: shortMessageDeleteDelay });
        return;
    }
    embedGenerator.sendNormalEmbed(message, "Recived invalid input", `Command only works when second argument is a number, recived ${message.content.split(' ')[1]}`);
};
