...
export const onCalled = (client: Client, message: Message) => {
    ...
    try{
        ...
        embedGenerator.sendNormalEmbed(
            message, 
            "Result: "+result,
            `Solved equation from ${message.member?.displayName} in ${Math.ceil(Date.now() - start)}ms`,
        )
    } catch (err:any) {
        embedGenerator.sendNormalEmbed(
            message,
            "Could not solve!",
            `Failed to solve equation from ${message.member?.displayName} in ${Math.ceil(Date.now() - start)}ms`,
            ["Reason: ", err.message]
        )
    }
}
